...
class BSCF7_Settings {
	...
	private $fields = [
		[
			'id' => 'autop-disable',
			'label' => ' Отключение AutoP',
			'description' => 'Отключение автоматической обертки всех полей в тег "p".',
			'type' => 'checkbox',
		],
		[
			'id' => 'wrapper-disable',
			'label' => 'Отключение враппера элементов',
			'description' => 'Внимание! Удаление враппера, нарушает работу функции валидации',
			'type' => 'checkbox',
		],
		[
			'id' => 'radio-checkbox',
			'label' => 'Чекбоксы / радио',
			'description' => 'Поддержка разметки Bootstrap 5 для чекбоксов и радио',
			'type' => 'checkbox',
		],
		[
			'id' => 'custom-css',
	...
	function settings_init() : void {
		...
		register_setting( 'bscf7-settings', 'bscf7_options' );


		// Register a new section.
		add_settings_section(
			'bscf7-settings-section',
			__( '', 'bscf7-settings' ),
			[$this, 'render_section'],
			'bscf7-settings',
			array(
				'before_section' => '<!-- postbox --><div class="postbox"><div class="inside">',
				'after_section' => '</div></div><!-- postbox END -->'
			)
		);
		...
		foreach( $this->fields as $field ) {
			...
			add_settings_field(
				...
				__( $field['label'], 'bscf7-settings' ), /* Label for the field. */
				[$this, 'render_field'], /* The name of the callback function. */
				'bscf7-settings', /* The menu page on which to display this field. */
				'bscf7-settings-section', /* The section of the settings page in which to show the box. */
				[
					'label_for' => $field['id'], /* The ID of the field. */
					'class' => 'bscf7_row', /* The class of the field. */
	...
	function options_page() : void {
		add_submenu_page(
			...
			'Настройки BSCF7', /* Page Title */
			'Настройки BSCF7', /* Menu Title */
			$this->capability, /* Capability */
			'bscf7-settings', /* Menu Slug */
	...
	function render_options_page() : void {
		...
		if ( isset( $_GET['settings-updated'] ) ) {
			...
			add_settings_error( 'bscf7_messages', 'bscf7_message', __( 'Настройки сохранены', 'bscf7-settings' ), 'updated' );
		}

		// show error/update messages
		settings_errors( 'bscf7_messages' );
		?>
		<div class="wrap">
			...
			<form action="options.php" method="post">
				<?php
				/* output security fields for the registered setting "bscf7" */
				settings_fields( 'bscf7-settings' );
				/* output setting sections and their fields */
				/* (sections are registered for "bscf7", each field is registered to a specific section) */
				do_settings_sections( 'bscf7-settings' );
	...
	function render_field( array $args ) : void {
		...
		$options = get_option( 'bscf7_options' );
		...
		switch ( $field['type'] ) {
			...
			case "text": {
				...
				<input
					...
					name="bscf7_options[<?php echo esc_attr( $field['id'] ); ?>]"
					value="<?php echo isset( $options[ $field['id'] ] ) ? esc_attr( $options[ $field['id'] ] ) : ''; ?>"
				>
				<p class="description">
					<?php esc_html_e( $field['description'], 'bscf7-settings' ); ?>
				</p>
				<?php
				break;
			}
			...
			case "wysiwyg": {
				wp_editor(
					...
					$field['id'],
					array(
						'textarea_name' => 'bscf7_options[' . $field['id'] . ']',
						'textarea_rows' => 5,
					)
				);
				break;
	...
	function render_section( array $args ) : void {
		?>
		<h3 id="<?php echo esc_attr( $args['id'] ); ?>"><?php esc_html_e( 'Управление плагином', 'bscf7-settings' ); ?></h3>
		<?php
	}
